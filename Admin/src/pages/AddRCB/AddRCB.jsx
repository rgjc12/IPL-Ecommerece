...
function AddRCB({token}) {
  ...
  const onSubmit = async(e)=>{
    ...
    try{
      ...
      const response = await axios.post(`${backendUrl}/api/rcbproduct/addrcb`,formData,{
        headers: {
      ...
      {
        toast.success('RCB Product Added Successfully!');
      }
      else 
      {
  ...
  return (
    <>
    <div id="addrcbmain">
      <form id="addrcbform" encType="multipart/form-data">
      <div id="uploadimages">
          <p>Upload Images:</p>
        ...
        <button id="addrcbbutton" type="submit" onClick={onSubmit}>Add </button>
      </form>
    </div>
    </>
  )
